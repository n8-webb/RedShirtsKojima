...
namespace Kojima
{
    ...
    public class CarScript : MonoBehaviour
    {
        ...
        public bool GetDrifting { get { return m_drifting; } }
        Vector3 m_driftVelo;
        ...
        void Start()
        {
            ApplyCarInfo(new CarInfo_s(100, 25, 15, 0.35f, CarInfo_s.driveMode_e.allWheels, "Engine1"));
        ...
        void Movement()
        {
            ...
            if (s_playersCanMove && m_canIMove)
            {
                ...
                //Rewired TODO:implement
                bool driftBtnPress = m_rewiredPlayer.GetButton("Drift");

                m_drifting = driftBtnPress && m_wheelIsGrounded[0] && m_wheelIsGrounded[1];
                ...
                for (int i = 0; i < m_wheels.Length; i++)
                {
                    ...
                    if (thisWheelCanDrive)
                    {
                        ...
                        if(moving)
                        {
                            ...
                            //rewired version
                            //Debug.Log(m_rewiredPlayer.GetAxis("Accelerator"));
                            Vector3 accelerationForce = direction * m_myInfo.m_facceleration * -(m_rewiredPlayer.GetAxis("Accelerator"));
                            Vector3 brakeForce = direction * m_myInfo.m_facceleration * (m_rewiredPlayer.GetAxis("Brake")) * 0.5f;
                    ...
                    if (i > 1)
                    {
                        float responsiveness = 1.8f;
                        //float 

                        float curTorqueSpeed = 0;
                        //curTorqueSpeed = m_myInfo.m_fturnSpeed * (m_fcurrentWheelSpeed / m_myInfo.m_facceleration);
                        //curTorqueSpeed = Mathf.Clamp(m_myInfo.m_fturnSpeed, 0, (m_fcurrentWheelSpeed));
                        curTorqueSpeed = (m_myInfo.m_fturnSpeed/m_myInfo.m_facceleration) * m_fcurrentWheelSpeed * responsiveness;
                        ...
                        {
                            //Rewired
                            m_rb.AddTorque(transform.up * m_rewiredPlayer.GetAxisRaw("Move Horizontal") * (curTorqueSpeed), ForceMode.Acceleration);
                        }
                    }
                }
            }
        ...
        void LateUpdate()
        {
            for (int i = 0; i < m_wheels.Length; i++)
            {
                ...
                if (i > 1)
                {
                    ...
                    float newY = Mathf.Lerp(-25, 25, lerpValue);
        ...
        void PreventSkidding()
        {
            ...
            float skidThreshold = 5.5f;

            if (Mathf.Abs(localVelo.x) > skidThreshold || Mathf.Abs(m_rb.angularVelocity.y) > skidThreshold * 2)
            {
